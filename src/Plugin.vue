  ...
  methods: {
    ...
    checkOptions() {
      if (typeof this.options.endpoint === "undefined") {
        ...
        console.error(
          `Please provide the option 'endpoint' with your endpoint url as value`
        );
      }
      if (typeof this.options.token === "undefined") {
        // eslint-disable-next-line
        console.info(
          `If you need an authentication using Basic Auth add a option with the name token.`
        );
        // eslint-disable-next-line
        console.info(
          `Your provided token will be used as username performing a Basic Auth, the password will be left blank`
        );
      }
    },
  },
